...
const RealResultsSection = () => {
  ...
  const handleAuthModalOpenChange = (open: boolean, modal: "login" | "signup") => {
    if (modal === "login") {
      setLoginOpen(open);
    } else {
      setSignupOpen(open);
  ...
  const InfiniteSlider = ({ items, direction = "left", speed = 50 }: { items: typeof partnerLogos, direction?: "left" | "right", speed?: number }) => {
    return (
